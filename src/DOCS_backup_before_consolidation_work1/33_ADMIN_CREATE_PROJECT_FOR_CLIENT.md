...
## 15. Session update (2026-08-25) — form field reorder, Add New Page quantity, Deleted Projects tab, payment history moved in-place, partial-payment "pending" bug fixed

Five separate, user-approved changes in one session. Read this before touching `CreateProjectForClientForm` (`AdminClientWorkspace.js`), `adminCreateProjectOrder.js`, `getOrderDetails.js`'s `hasUnpaidInvoice` query, or the `AdminClientWorkspace.js` Payments tab / `AdminPaymentRecordDetail.js`.

### 15.1 `CreateProjectForClientForm` — field order + Add New Page quantity (UI + backend)

**Before**: field order was Starting Node Title (required) → Category → Total Pages (website categories only) → Base Price (read-only display) → Reference Total → Selling Price → Additional Features (toggle-only, no quantity) → Payment Type. No "Project Name" field existed (removed in an earlier session, doc §6 table).

**After**:
- Field order is now **Category → Project Name (new, optional, falls back to category label) → Starting Node Title (now optional, falls back to `"Project Started"`) → Additional Features + Selling Price (side-by-side in one row) → Reference Total (moved to just above Payment Type) → Payment Type**.
- **Total Pages standalone field removed** — but the underlying `totalPages`/`projectSnapshot.totalPages` backend field was **not** removed (a scope-correction; see the note below).
- **"Add New Page" feature gained a quantity stepper** (+/-, styled after the existing customer-facing `DraftOrderCard.js` "Add More Pages" white-box pattern) inside the features dropdown's selected-chips row. Only the product named exactly `"Add New Page"` (`ADD_NEW_PAGE_FEATURE_NAME` constant, matched by `feature.serviceName`) gets the stepper; every other `feature_upgrades` product stays a plain toggle.
- **Backend (`adminCreateProjectOrder.js`)**: new `projectName` and `featureQuantities` request fields. `startingNodeTitle` validation relaxed (empty/missing → server-side default `DEFAULT_STARTING_NODE_TITLE = "Project Started"`, also used by `initializeProjectTimeline`). `clientProjectFeatures` now carries a `quantity` field; for the Add New Page feature, `price = unitPrice × quantity` (server-recomputed, quantity never trusted beyond `Math.max(1, ...)`). `orderItems[].quantity` is now dynamic per feature instead of hardcoded `1`.

**Correction recorded (important — do not repeat)**: the user asked only to remove the **frontend** Total Pages input field (superseded by the Add New Page quantity system). A first pass over-scoped this into removing the **backend** `totalPages` handling too (request destructuring, validation, `projectSnapshot.totalPages` write) — `totalPages` is a real pre-existing schema field on `orderProductModel.js`/`productModel.js`, read by `getOrderDetails.js` and other display code, so removing it broke that field for new admin-created projects. The user caught this and asked for double-confirmation before any destructive change going forward — the backend `totalPages` field/param was restored in the same session. See the memory `feedback_destructive_scope_confirm.md` for the standing rule this created.

### 15.2 New "Deleted Projects" tab — `AdminClientWorkspace.js` (per-client)

**Before**: `deleteOrder.js` hard-deletes an order (`orderModel.deleteOne`) but does **not** delete its `transactionModel`/`invoiceModel` records (only `updateRequestModel`, `monthlyInvoiceModel`, `transactionModel`, `partnerCommissionModel` are cascaded — `invoiceModel` was never in that cascade, confirmed, a separate known gap not fixed this session). `getAdminUserWorkspace.js` already flagged these orphaned records `orderDeleted: true` (`plain.orderDeleted = Boolean(rawOrderId) && !plain.orderId`), and `paymentLedger.js`'s `groupLedgerItemsByProject()` already had an `orderDeleted`-aware "Deleted Project" fallback label — but nothing in the UI used that flag to separate deleted-project payment history from active-project history; it showed mixed into the same Payment & Invoices tab list.

**After**: `WORKSPACE_TABS` gained a `DELETED_PROJECTS_TAB` (between Payments and Documents). `PaymentInvoicesPanel`'s `ledgerGroups` now excludes any group where `item.orderDeleted` is true (and its summary pills — Paid/Pending/Unpaid/Paid counts — now filter `!invoice?.orderDeleted`/`!transaction?.orderDeleted` too). A new `DeletedProjectsPanel` component (read-only, no recharge/actions — the project itself no longer exists) renders exactly those excluded groups. No backend change — `orderDeleted` was already being computed; this was purely a frontend split.

### 15.3 Payment history — moved from a routed page to an in-place subpage

**Before**: clicking a payment group card in the Payments tab called `navigate(...)` to `/admin-panel/clients/:customerId/payments/order/:orderKey`, which fully left `AdminClientWorkspace.js` and rendered `AdminPaymentRecordDetail.js`'s `PaymentOrderHistory` component (its own `AdminLayout` wrap, full page load/URL change) — inconsistent with how Projects/Plans subpages work (`activeProjectId`/`activePlanId` state-driven, no navigation, "Back" is instant).

**After**: `AdminClientWorkspace.js` gained `activePaymentGroupId` state, following the exact same pattern as `activeProjectId`/`activePlanId` (`handleTabChange`/`handleBack` both know to close it first). `PaymentOrderHistory`'s full logic (data fetch, invoice/transaction lists, final-invoice actions, payment-review modal) was ported into a new `PaymentOrderHistorySubpage` component **inside `AdminClientWorkspace.js`**, `AdminLayout` wrapper removed (parent is already inside one). `AdminPaymentRecordDetail.js` itself was **not deleted or modified** — it remains a standalone routed page for any direct/bookmarked link, but is no longer how this workspace's own Payments tab opens a project's history. **Explicitly out of scope, confirmed with the user**: the deeper "Open record" single-transaction/invoice detail view (`AdminPaymentRecordDetail.js`'s `SinglePaymentRecordDetail`) still navigates to its own route — only the group/order-level history was converted to a subpage.

### 15.4 Invoice + transaction shown as one payment event, not two — combined ledger rows

**Root cause (found via a read-only trace before any fix)**: the write side was always correct — `paymentRecording.js`'s `markProjectInvoicePaid()` links every transaction it creates/reuses to its invoice (`transaction.invoiceId = invoice._id`). A dedup helper for this already existed and was correctly used elsewhere: `paymentLedger.js`'s `buildLedgerItems()` drops an invoice from its output if a transaction with a matching `invoiceId` is already present (used by `PaymentInvoicesPanel`'s main list and the per-order ledger inside project detail). **But `PaymentOrderHistorySubpage` (and its `AdminPaymentRecordDetail.js` twin, same bug, not fixed since that file is not the active path per §15.3) never called `buildLedgerItems()`** — it fetched raw `invoices` and raw `transactions` and rendered them as two independent, unfiltered sections ("Invoices" and "Payment Requests & History"), so one real payment (its invoice + its linked transaction) always appeared as two separate rows.

**Fix**: `PaymentOrderHistorySubpage`'s `useMemo` now also derives `combinedRecords` — every invoice matched against `matchingTransactions` by `transaction.invoiceId === invoice._id` (a `Map` keyed by invoice id); an invoice with a match renders as **one row** showing the transaction's status/method/reference (the actual payment attempt takes priority) alongside the invoice's own number/due-date; an invoice with no transaction yet, or a transaction with no `invoiceId` (e.g. a wallet-only general payment), each still get their own row. Action buttons (Review Payment / Review & Record Payment / Review Initial Payment) attach to whichever side of the merged row is actually actionable. **`paymentLedger.js`/`buildLedgerItems()` itself was not touched** — this fix is local to `PaymentOrderHistorySubpage`'s own row-building, not a change to the shared helper.

### 15.5 `PaymentOrderHistorySubpage` UI — document-style single card, per-row invoice actions, project payment summary

Follow-up polish pass on §15.3/§15.4, same file:
- Replaced the multi-box layout (separate bordered panels for header / "Live cumulative statement" final-invoice box / payment-records list) with **one continuous card**, sections separated by `border-t` dividers only — matching the single-card-with-dividers pattern already established on `ProjectServiceWorkspace.js`.
- **New header summary block**: project category/phase/start-date (`getCategoryLabel`/`getPhaseLabel`/`getProjectCategory`, reused from the Projects-tab helpers already in this file) plus a payment summary (total amount, paid, remaining, payment type Full/Partial, installment progress `"Installment N of M"` when partial) — all derived from the already-fetched `order` object, no new API call.
- **"Live cumulative statement" floating box removed.** Its View/Download/Share/Email actions now render **inline on the final-invoice's own row** inside the combined records list (§15.4), instead of a separate box above an actionless list.
- **Regular (non-final) invoices gained their own Download/Email actions**, via two new handlers (`handleInvoiceDownload`/`handleInvoiceResend`) calling the pre-existing `SummaryApi.adminPaymentRecord`'s `download-invoice`/`resend-invoice` endpoints (`backend/controller/invoice/monthlyInvoiceController.js`'s `downloadPaymentRecordInvoice`/`resendPaymentRecordInvoice`, which work for any invoice via `recordType: "invoice"`, not just final invoices) — these endpoints already existed (used by `AdminPaymentRecordDetail.js`'s `SinglePaymentRecordDetail`) but were not wired into this subpage before.

### 15.6 Partial-payment "Payment Pending" false-positive after a correctly-approved installment (bug fix)

**Root cause (read-only trace, then fixed)**: `getOrderDetails.js`'s `hasUnpaidInvoice`/`unpaidInvoice` (consumed only by `ProjectDetails.js`, which gates the customer-facing "Payment Pending" banner and the upload-lock) queried for **any** `unpaid`/`overdue` invoice on the order, with no due-date or progress-threshold awareness. For a partial-payment order, once installment #1 is approved, installment #2's invoice is naturally still `unpaid` (it isn't due yet — its `progressThreshold`, e.g. 90% progress, hasn't been reached) — but the unscoped query matched it anyway, so the customer saw "Payment Pending" reappear immediately after a correctly-approved payment. Confirmed **not** a write-side bug: `markProjectInvoicePaid()`, `settleInstallmentInvoice()`, and `transactionApprovalController.js`'s `applyOrderMoneyForTransaction` all write consistent, correct state.

**Contributing factor specific to `adminCreateProjectOrder.js`**: for admin-created partial-payment projects, **all** installment invoices are created `unpaid` up-front in one loop at project-creation time (not due-based, unlike `customerCreateCustomProjectOrder.js`'s find-or-create-on-attempt pattern) — so this bug's window opened immediately after installment #1's approval for this creation path, rather than only once a later installment was actually attempted.

**Fix**: `getOrderDetails.js`'s unpaid-invoice query is now scoped, for orders with a non-empty `installments` array, to `installmentNumber: order.currentInstallment` — and additionally excluded (`installmentNumber: -1` sentinel, never matches) if that installment has a configured `progressThreshold` not yet reached by `order.projectProgress`. One-time (non-partial) orders have no `installments` array, so the added condition is skipped entirely and their single-invoice check is unchanged. **Owner-confirmed design decision**: "Payment Pending" must reflect only a payment the customer can act on right now — a future, not-yet-due installment must never surface it.

### Files touched (this update)

- **Backend**: `backend/controller/order/adminCreateProjectOrder.js` (§15.1 — `projectName`, `featureQuantities`, `DEFAULT_STARTING_NODE_TITLE`, `totalPages` restored); `backend/controller/order/getOrderDetails.js` (§15.6 — `unpaidInvoiceFilter` scoping).
- **Frontend**: `frontend/src/pages/AdminClientWorkspace.js` (all of §15.1–15.5 — `CreateProjectForClientForm`, `WORKSPACE_TABS`/`DeletedProjectsPanel`, `PaymentOrderHistorySubpage` new, `handleOpenPaymentGroup`/`handleTabChange`/`handleBack` updated).
- **Not touched**: `AdminPaymentRecordDetail.js` (still live for direct/bookmarked links, not edited), `paymentLedger.js` (dedup helper untouched, reused conceptually not literally), `leadModel.js`/lead-side files (unrelated to this update — see §10 above for those).
- No `npm run build` run, per standing instruction.
