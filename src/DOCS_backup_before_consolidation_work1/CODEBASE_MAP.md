...
- Orphaned records from hard-deleted orders (`orderDeleted: true`) live in a separate, read-only **Deleted Projects** tab — caused by `deleteOrder.js` not cascading `invoiceModel` (only cascades `updateRequestModel`/`monthlyInvoiceModel`/`transactionModel`/`partnerCommissionModel` — known, unfixed gap).
...
- **Known gap**: `deleteOrder.js` cascades `updateRequestModel`/`monthlyInvoiceModel`/`transactionModel`/`partnerCommissionModel` but **not** `invoiceModel` — produces orphaned invoice/transaction records, surfaced read-only in the "Deleted Projects" tab.
...
- `deleteOrder.js` doesn't cascade `invoiceModel` (§13) — orphaned records possible.
...
- `invoiceLifecycle.js`'s `resumeOrderForPaidInvoice()` resumes a whole order/plan when one overdue invoice is marked paid, without checking if a different invoice on the same order is still overdue.
