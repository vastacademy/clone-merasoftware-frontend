...
- `/admin-panel/leads` - `AdminLeadsPage` (Lead/CRM list; prospects before they become customers — new `leadModel`; add-lead modal, phone required; source-filter badges All/Normal/Guest + independent status-filter badges All/Matured/Not Matured — "Matured" is a **display-only** rename of the stored `"Won"` status, DB/enum value unchanged. See `43_LEAD_CRM_SYSTEM_PHASE_1_TO_6A.md`)
...
- `AdminClientWorkspace` has `Projects`, `Plans`, `Payments`, `Deleted Projects`, `Documents`, `Account & Access`, `Overview` tabs (fixed order)
...
- The `Payments` tab (`PaymentInvoicesPanel`) uses existing workspace `transactions` and `invoices` arrays; it does not create a new admin payment source
- The `Payments` tab shows payment records as display-only until transaction approve/reject backend routes are verified/completed
- The `Payments` tab lets admin mark `unpaid` and `overdue` invoices as paid through `/api/invoices/:invoiceId/mark-paid`
- The `Payments` tab excludes orphaned payment records from **hard-deleted** projects (`orderDeleted: true`, set by `getAdminUserWorkspace.js` when a transaction/invoice's `orderId` no longer resolves) — those live in the separate `Deleted Projects` tab (`DeletedProjectsPanel`), read-only, no recharge/actions. `deleteOrder.js` hard-deletes the order but does **not** cascade `invoiceModel` (only `updateRequestModel`/`monthlyInvoiceModel`/`transactionModel`/`partnerCommissionModel` — a known, unfixed gap), which is why these orphaned records exist at all.
- Clicking a payment group card in the `Payments` tab opens an **in-place subpage** (`PaymentOrderHistorySubpage`, state-driven via `activePaymentGroupId`, same pattern as `activeProjectId`/`activePlanId` — no route navigation, "Back" is instant) showing that project/plan's full payment history as **one combined row per real payment event** (an invoice and its linked transaction, matched by `transaction.invoiceId`, render as a single row — never two separate rows for the same payment). The older routed page `AdminPaymentRecordDetail.js`'s `PaymentOrderHistory`/`SinglePaymentRecordDetail` still exists for direct/bookmarked links but is no longer how this workspace opens a group's history; only the group/order-level view was converted to a subpage, not the deeper single-record ("Open record") view.
...
- `CreateProjectForClientForm` (`AdminClientWorkspace.js`) is now a 2-step flow — Project Details, then a "Payment Settings" step where admin either records the first payment immediately (`recordPayment` sent to `adminCreateProjectOrder.js`, which marks the first invoice paid via a new `markProjectInvoicePaid()` helper — not the recurring-plan-only `invoiceLifecycle.js`) or explicitly defers it ("Just Add Project, Let Client Pay the Bill"). `getOrderDetails.js` now returns `hasUnpaidInvoice`/`unpaidInvoice`; `ProjectDetails.js` (customer-side) shows a "Payment Pending" banner and disables "Request Update" while the project's invoice is unpaid. See `38_TWO_STEP_PAYMENT_SETTINGS_AND_PAYMENT_PENDING_LOCK.md`. **Fixed (2026-08-25)**: for partial-payment orders, `hasUnpaidInvoice` is now scoped to only `order.currentInstallment`'s invoice (and further gated on that installment's `progressThreshold` being reached by `order.projectProgress`) — previously it matched *any* unpaid invoice on the order, so a not-yet-due future installment's naturally-unpaid invoice would re-trigger the "Payment Pending" banner immediately after an earlier installment was correctly approved. One-time orders (no `installments` array) are unaffected. See `33_ADMIN_CREATE_PROJECT_FOR_CLIENT.md` §15.6.
