...
const CreateTicket = ({ show, handleClose, refreshTickets }) => {
  ...
  const fieldClassName = 'w-full rounded-xl border border-[var(--glass-border-strong)] bg-[var(--glass-bg-subtle)] px-3 py-2.5 text-base text-[var(--text-primary)] outline-none placeholder:text-[var(--text-muted)] transition focus:border-emerald-400 focus:ring-4 focus:ring-emerald-400/20';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--glass-bg-subtle)] px-4 py-6 backdrop-blur-sm">
      <div role="dialog" aria-modal="true" aria-labelledby="create-ticket-title" className="flex max-h-full w-full max-w-xl flex-col overflow-hidden rounded-[1.75rem] border border-[var(--glass-border-strong)] bg-[var(--menu-bg)] text-[var(--text-primary)] shadow-2xl backdrop-blur-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-[var(--glass-border)] px-5 py-5 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <div><h2 id="create-ticket-title" className="text-xl font-bold">Create support ticket</h2><p className="mt-1 text-sm text-[var(--text-secondary)]">Describe the issue and our team will review it.</p></div>
          </div>
          <button type="button" onClick={closeModal} className="rounded-xl border border-[var(--glass-border)] bg-[var(--glass-bg-subtle)] p-2 text-[var(--text-primary)] transition hover:bg-[var(--glass-bg)]" aria-label="Close"><X className="h-4 w-4" /></button>
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5 sm:px-6">
          {hasOpenTicket ? (
            <div className="rounded-2xl border border-amber-400/30 bg-amber-500/10 p-4 text-amber-100"><div className="flex items-center gap-2 font-semibold"><AlertCircle className="h-5 w-5" />You already have an open ticket</div><p className="mt-2 text-sm text-amber-100/80">Please wait for resolution before creating a new ticket.</p><button type="button" onClick={openExistingTicket} className="mt-4 rounded-xl bg-emerald-500 px-4 py-2.5 text-sm font-semibold text-[var(--text-primary)] transition hover:bg-emerald-400">View existing ticket</button></div>
          ) : (
        ...
        {!hasOpenTicket && <div className="flex flex-col-reverse gap-2 border-t border-[var(--glass-border)] px-5 py-5 sm:flex-row sm:justify-end sm:px-6"><button type="button" onClick={closeModal} disabled={loading} className="rounded-xl border border-[var(--glass-border-strong)] bg-[var(--glass-bg-subtle)] px-4 py-2.5 text-base font-semibold text-[var(--text-primary)] transition hover:bg-[var(--glass-bg)] disabled:opacity-50">Cancel</button><button type="submit" form="create-ticket-form" disabled={loading} className="inline-flex items-center justify-center rounded-xl bg-emerald-500 px-4 py-2.5 text-base font-semibold text-[var(--text-primary)] transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:bg-[var(--glass-bg-strong)]">{loading && <span className="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" />}{loading ? 'Submitting…' : 'Submit ticket'}</button></div>}
      </div>
    </div>
  );
};
