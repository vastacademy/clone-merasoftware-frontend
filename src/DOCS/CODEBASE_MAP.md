...
### Hybrid parent-child payment (multi-service wallet+UPI/combined)
...
- Solution: one **parent** transaction (no `orderId`, no `invoiceId`) holds the money the admin approves; one **child** per service carries its own `orderId`/`invoiceId` + `parentTransactionId`. Approving the parent settles every child through the unchanged single-order path (`applyApprovedOrderPayment`). Reject refunds every wallet portion.
- Split decided server-side: `walletPart = min(balance, total)`, wallet allocated sequentially across services (whole-rupee shares). Fully wallet-covered ⇒ instant activation, no parent created. Any UPI remainder ⇒ all services `pending-approval` together.
- `createPaymentTransaction` takes explicit `type`/`sourceType`/`paymentStatus` (must not let a no-`orderId` parent get misclassified as a wallet `deposit`).
...
### Three-axis service design — schema is now live, enforcement is not
The `timing`/`dependency`/`capability` schema fields (§ above) are already implemented in `productModel.js`, closing what doc 56 called "Phase A — not yet built." What is **still genuinely unbuilt** (no evidence found otherwise): the activation engine (a dormant `after`-timing service activating at 100% progress), upload-unlock/enforcement actually reading these fields, project-click routing logic using them in the shared list helpers, reminder delivery + history schema, and the cycle/recurring-billing engine. No admin UI exists to view/pause/cancel a customer's live service.
Key unbuilt-but-designed rules (still just design intent): purchase ≠ activation (an `after`-timing service bought early must lie dormant, validity counted from activation, not purchase); "control" means unlock (without the service, the upload action shouldn't exist at all; buying it turns it on with a limit). See `plansystem.md` §5.3/§6 for the full recurring-billing design (grace period, pause-not-cancel model) — still the only source for that unbuilt work.
...
- Plans/recurring stay on the separate `monthlyInvoiceModel` (untouched, different system — do not merge).
- `invoiceModel.js` confirmed fields: `userId`, `orderId`, `invoiceNumber` (unique), `invoiceType` enum `["project", "project_final", "plan_renewal"]` (a partial unique index enforces one `project_final` per `orderId`), `amount`, `amountPaid` (running total, default 0), `status` enum `["unpaid","partially_paid","paid","overdue","cancelled"]`, `invoiceDate`, `dueDate`, `paidDate`, `installmentNumber`, `lineItems[]`, `paymentMethod`, `transactionReference`, `notes`, `internalNote`, `markedPaidBy`.
...
- One canonical pay surface: `InvoiceDetailPage.js`'s "Pay Now" (in-page wallet/UPI-QR, posts `/wallet/verify-payment` with `invoiceId`+`orderId`+`sourceType:'invoice'`). Every other surface (`ProjectDetails.js`'s "Payment Pending" banner, rejected-order "Retry Payment") links to `/invoice-detail/:invoiceId` rather than duplicating payment UI.
...
- **Bulk add-on-service batch payments now show the linked project's name instead of a bare "Payment" label.** Was: `customerCreateServicePlanOrdersBulk.js`'s parent transaction (§5's hybrid parent-child pattern) deliberately carries no `orderId`/`invoiceId` — settling must stay delegated to its children, so the parent never touches a project order directly. But `paymentLedger.js`'s `getOrderDisplayName()`/`getTransactionPaymentLabel()` only ever read `transaction.orderId`, so a batch payment's only project reference (`paymentDetails.linkedProjectOrderId`, display-only, never read anywhere) was invisible — the ledger fell through to the generic `"Payment · UPI"` label, which read like an unlabeled/wallet-recharge-style entry. Now: `getAdminUserWorkspace.js` selects `paymentDetails` on transactions and, for any transaction with no `orderId` but a `paymentDetails.linkedProjectOrderId`, batch-fetches that order's display snapshot (`projectSnapshot`/`productId.serviceName`/`servicePlanSnapshot`/`orderItems`, same fields already populated on `orderId` elsewhere) into a new `linkedProjectSnapshot` field — settlement/approval logic is untouched, this is display-only. `paymentLedger.js`'s `buildLedgerItems()` falls back to `getOrderDisplayName(transaction.linkedProjectSnapshot)` when `transaction.orderId` is empty; `getTransactionPaymentLabel()` gets a new `paymentDetails?.isServiceBatch` branch returning `"N Service(s) · <method>"` instead of falling through to the generic label. **Known gap, not fixed**: if the linked project was later hard-deleted, `linkedProjectSnapshot` resolves to nothing (no name-snapshot was ever saved on `paymentDetails`, unlike the invoice-level `deletedProjectName` in the "Deleted Projects" entry above) — the payment then shows only the generic `"N Service(s) · <method>"` label with no project name. Accepted as rare/low-impact rather than fixed.
...
- **3-tier glass pattern** (established, reused across most customer-portal pages):
  ...
  Exception: payment-input forms (wallet recharge modal, `DirectPayment.js`/`InstallmentPayment.js` internals) stay solid, not glassed.
...
- `createOrder.js` (the older, still-live public-storefront order path) still creates all-installments-up-front invoices rather than due-based — inconsistent with the due-based rule used by the newer creation paths.
...
| Service hybrid payment | `backend/controller/order/customerCreateServicePlanOrdersBulk.js`, `backend/controller/user/transactionApprovalController.js`, `components/AddServiceModal.js` |
