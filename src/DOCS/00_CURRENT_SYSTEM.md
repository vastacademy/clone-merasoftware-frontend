...
- `/my-updates`
- `/my-invoices`
- `/documents` - `CustomerDocuments` (new; one newest-first timeline of every admin-sent document — lead-stage `proposals[]` + client-stage `userModel.documents[]` — via `GET /api/my-documents`; read-only, admin-sent files only. See `50_CLIENT_DOCUMENTS_SYSTEM.md`)
- `/direct-payment`
- `/support`
