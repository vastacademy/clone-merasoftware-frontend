...
const AdminPlanProductsPage = () => {
  ...
  // "Delete Forever", only offered on the Retired tab. Archives by default; the
  // privileged hard delete additionally requires the plan's name to be retyped.
  ...
  const handleConfirmPurge = async () => {
    ...
    setPurging(true);
    try {
      const response = await fetch(SummaryApi.purgePlan.url(purgeTarget._id), {
        method: SummaryApi.purgePlan.method.toUpperCase(),
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mode: hardDelete ? "hard" : "archive",
          confirmName: hardDelete ? confirmName.trim() : undefined,
        }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.message || "Plan could not be deleted.");
      ...
      toast.error(purgeError.message || "Plan could not be deleted.");
    } finally {
      setPurging(false);
    }
  };
  ...
  return (
    <AdminLayout user={user} onLogout={handleLogout}>
                ...
                <div className="col-span-6 flex items-center justify-end gap-2 lg:col-span-1">
                  {plan.retiredAt ? (
                    <>
                      ...
                      >
                        {changingServiceId === plan._id ? "..." : "Restore"}
                      </button>
                      <button
                        type="button"
                        onClick={(event) => { event.stopPropagation(); setPurgeTarget(plan); setHardDelete(false); setConfirmName(""); }}
      ...
      {/* Delete Forever — two levels. Default archives (recoverable in the DB but
          gone from every list); the privileged hard delete destroys the row and
          demands the plan's exact name first. */}
      {purgeTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-slate-950">
              Delete "{purgeTarget.serviceName}" forever?
            </h3>
            <p className="mt-3 text-sm text-slate-600">
              It disappears from every list, including Retired. Existing purchases,
              invoices and running services keep their own record and are not affected.
            </p>

            <label className="mt-5 flex cursor-pointer items-start gap-2 rounded-xl border border-rose-200 bg-rose-50 p-3">
              <input
                type="checkbox"
                checked={hardDelete}
                onChange={(event) => { setHardDelete(event.target.checked); setConfirmName(""); }}
                className="mt-0.5"
              />
              <span className="text-xs text-rose-800">
                <strong>Also erase it from the database.</strong> This cannot be undone —
                without it the plan stays recoverable by an administrator.
              </span>
            </label>
            ...
            {hardDelete && (
              <label className="mt-3 block">
                <span className="text-xs font-semibold text-slate-700">
                  Type <strong>{purgeTarget.serviceName}</strong> to confirm
                </span>
                <input
                  type="text"
                  value={confirmName}
                  onChange={(event) => setConfirmName(event.target.value)}
                  className="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:border-rose-400 focus:ring-2 focus:ring-rose-100"
                />
              </label>
            )}
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => { setPurgeTarget(null); setHardDelete(false); setConfirmName(""); }}
                disabled={purging}
                className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
              >
                Cancel
              ...
              <button
                ...
                disabled={purging || (hardDelete && confirmName.trim() !== purgeTarget.serviceName)}
              ...
              >
                {purging ? "Deleting..." : hardDelete ? "Erase permanently" : "Delete Forever"}
              </button>
            </div>
          </div>
        </div>
