  ...
  return (
    <div className="relative shrink-0" ref={boxRef}>
      <button
        ...
        className="flex items-center gap-2 rounded-xl px-2 py-1.5 text-[var(--text-secondary)] transition hover:bg-[rgb(var(--tint-rgb)/0.1)] hover:text-[var(--text-primary)]"
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 top-11 z-50 w-64 overflow-hidden rounded-2xl border border-[rgb(var(--line-rgb)/0.12)] bg-[var(--menu-bg)] py-1.5 shadow-xl"
        >
          ...
          {themes.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-start gap-3 px-4 py-2.5 text-left transition hover:bg-[rgb(var(--tint-rgb)/0.1)] ${
