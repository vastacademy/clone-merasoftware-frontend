    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className={`portal-surface absolute inset-y-0 right-0 flex w-72 max-w-[85vw] flex-col shadow-2xl ${c("bg-slate-950 text-white", "text-[var(--text-primary)]")}`}>
        <button
          ...
          className={`absolute left-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full ${c("bg-white/10 text-white hover:bg-white/20", "bg-[rgb(var(--tint-rgb)/0.1)] text-[var(--text-primary)] hover:bg-[rgb(var(--tint-rgb)/0.2)]")}`}
        >
          <X size={18} />
        </button>
