...
const PortalHeader = ({ user, portalLabel = "Portal", dashboardTo = "/dashboard", onLogout, showProfileLink = true, showThemeSwitch = false, links = [] }) => {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b backdrop-blur ${c("border-white/10 bg-slate-950/95", "border-[rgb(var(--line-rgb)/0.1)] bg-[var(--header-bg)]")}`}>
              ...
              <Link
                ...
                className={`whitespace-nowrap rounded-xl px-3 py-2 text-sm font-semibold transition ${
                  isLinkActive(to)
                    ...
                    : c("text-slate-300 hover:bg-white/10 hover:text-white", "text-[var(--text-secondary)] hover:bg-[rgb(var(--tint-rgb)/0.1)] hover:text-[var(--text-primary)]")
                }`}
              >
                {label}
              </Link>
        ...
        >
          {user ? (
            <button
              ...
              className={`flex items-center gap-2 rounded-xl px-2 py-1.5 transition ${c("hover:bg-white/10", "hover:bg-[rgb(var(--tint-rgb)/0.1)]")}`}
            ...
            >
              {user?.profilePic ? (
                ...
                <span className={`flex h-9 w-9 items-center justify-center rounded-full border-2 text-sm font-bold ${c("border-white/15 bg-white/10 text-white", "border-[rgb(var(--line-rgb)/0.15)] bg-[rgb(var(--tint-rgb)/0.1)] text-[var(--text-primary)]")}`}>
                  {userInitial}
                </span>
              )}
          ...
          {menuOpen && user ? (
            <div className={`absolute right-0 top-12 w-56 rounded-2xl border p-2 shadow-xl ${c("border-white/10 bg-slate-900", "border-[rgb(var(--line-rgb)/0.12)] bg-[var(--menu-bg)]")}`}>
              <div className={`border-b px-3 py-2 ${c("border-white/10", "border-[rgb(var(--line-rgb)/0.1)]")}`}>
              ...
              <Link
                to={dashboardTo}
                className={`mt-1 block rounded-xl px-3 py-2 text-sm font-medium ${c("text-slate-200 hover:bg-white/10", "text-[var(--text-secondary)] hover:bg-[rgb(var(--tint-rgb)/0.1)]")}`}
