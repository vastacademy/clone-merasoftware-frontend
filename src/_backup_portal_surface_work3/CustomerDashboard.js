...
const MetricCard = ({ icon: Icon, label, value, helper, tone = 'neutral', to, state, highlight = false }) => {
  ...
  // a number, not a status, so it no longer tries to mean anything: emerald
  // only where the card is genuinely highlighted, neutral otherwise. Light mode
  // hid this problem rather than fixing it, since --glow-opacity is 0.14 there.
  const glowMap = {
    neutral: 'bg-[rgb(var(--tint-rgb))]',
  ...
  return (
    <Surface
      ...
      className={[
        ...
        highlight
          ? 'border-[var(--badge-success-border)] hover:-translate-y-1 hover:bg-[var(--glass-bg-hover)]'
          : 'hover:border-[var(--glass-border-strong)] hover:bg-[var(--glass-bg-hover)]',
        to ? 'block' : '',
      ].join(' ')}
    >
      <div
        className={`pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full ${highlight ? 'bg-emerald-400' : (glowMap[tone] || glowMap.neutral)} blur-3xl`}
      ...
      <div className="relative flex items-start justify-between gap-4">
        ...
        <div className={`rounded-2xl border p-3 backdrop-blur-md ${highlight ? 'badge badge-success' : 'border-[var(--glass-border-strong)] bg-[var(--glass-bg)]'}`}>
